...
const RealtimeData: React.FC = () => {
    ...
    const filteredData = tableData.filter((row) => {
        ...
        if (!isLocateView && frameTypeFilter !== "all" && rowFrame !== frameTypeFilter)
            return false;
...
{JSON.stringify(rec, null, 2)}
    ...
    const totalCount = isLocateView ? locateList.length : tableData.length;
    const displayCount = isLocateView
        ? locateFiltered.length
        : filteredData.length;
                        ...
                        >
                            ...
                            <div>
                                ...
                                <Input
                                    value={macFilter}
                                    onChange={(e) =>
                                        setMacFilter(e.target.value.trim())
                                    }
                ...
                <Col xs={24} md={21}>
                    ...
                    >
                        <Table<any>
                            ...
                            pagination={false}
                            // BLE 列表限制高度，locate 不限制，方便看完整 JSON
                            scroll={{ y: 750 }}
                            style={{ tableLayout: "fixed" }}
                            rowKey={(row: any) =>
                                row.key ?? `${row.mac}_${row.ts ?? ""}`
                            }
                        />
                    </Card>
                </Col>
